...
DROP PROCEDURE IF EXISTS GenerateSlotsSafely;
...
BEGIN
    ...
    ELSEIF p_start_time >= p_end_time THEN
        SET p_error_message = CONCAT('Start time ', p_start_time, ' must be before end time ', p_end_time);
        SET p_generated_count = 0;
        ROLLBACK;
    ...
    ELSE
        ...
        SET v_total_minutes = TIME_TO_SEC(p_end_time) - TIME_TO_SEC(p_start_time);
        SET v_total_minutes = v_total_minutes / 60;
        SET v_total_slots = FLOOR(v_total_minutes / p_slot_duration_minutes);
        ...
        ELSE
            ...
            slot_loop: WHILE v_current_time < p_end_time DO
                SET v_slot_end_time = ADDTIME(v_current_time, CONCAT(FLOOR(p_slot_duration_minutes / 60), ':', LPAD(p_slot_duration_minutes % 60, 2, '0'), ':00'));
                ...
                SET v_current_time = ADDTIME(v_current_time, CONCAT(FLOOR(p_slot_duration_minutes / 60), ':', LPAD(p_slot_duration_minutes % 60, 2, '0'), ':00'));
            END WHILE slot_loop;
